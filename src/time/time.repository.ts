...
import {IGetTime, ISaveTime, ITimeRepository } from "./time.repository.interface";
...
export class TimeRepository implements ITimeRepository{
    ...
    async saveTime(payload:ISaveTime): Promise<any | null>{
    ...
    async getTime(payload:IGetTime): Promise<any | null>{
    ...
    async getTimeAll(payload: GetTimeAllDto): Promise<any | null>{
    ...
    async allTimeBySelectedDate(user_id:number, date:string):Promise<any | null>{
        return await db.query(
    ...
    async getAllUserTime(user_id:number):Promise<any>{
